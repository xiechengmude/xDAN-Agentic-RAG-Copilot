...
class EnhancedS3RAGServiceV2:
    ...
    def __init__(self, ragflow_client: Union[RAGFlowSDKWrapper, RAGFlowClient, StreamingRAGFlowClient], 
        ...
        self.is_streaming_mode = isinstance(ragflow_client, StreamingRAGFlowClient)
        
        logger.info(f"Enhanced S3 RAG Service V2 初始化 - SDK模式: {self.is_sdk_mode}, 流式模式: {self.is_streaming_mode}")
    ...
    def search_step(self, question: str, dataset_ids: List[str], top_k: int = 5, 
        ...
        try:
            ...
            if self.is_streaming_mode:
                # Use streaming client's retrieve method
                response = self.ragflow_client.retrieve(
                    question=question,
                    dataset_ids=dataset_ids,
                    top_k=top_k
                )
                logger.info(f"流式客户端检索响应代码: {response.get('code')}")
                if response.get('code') == 0:
                    chunks = response.get('data', {}).get('chunks', [])
                    logger.info(f"流式检索成功，找到 {len(chunks)} 个chunks")
                    return chunks, True
                else:
                    logger.error(f"流式检索失败: {response.get('message')}")
                    return [], False
            elif self.is_sdk_mode:
