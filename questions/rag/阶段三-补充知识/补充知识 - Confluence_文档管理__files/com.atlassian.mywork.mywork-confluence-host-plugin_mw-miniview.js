...
MW.$(function(A){(function(B,C){MW.$.effects.slide=function(D){return this.queue(function(){var G=MW.$(this),F=["position","top","bottom","left","right"];
var K=MW.$.effects.setMode(G,D.options.mode||"show");
var J=D.options.direction||"left";
MW.$.effects.save(G,F);
G.show();
MW.$.effects.createWrapper(G).css({overflow:"hidden"});
...
MW.Deferred=function(){var A=MW.$.Deferred();
...
MW.$(function(){MW.$.fn.visible=function(){var A=MW.$(this);
return !!(A.width()||A.height())&&A.css("display")!=="none"
};
if(!_.groupBy){MW.$.extend(_,{groupBy:function(B,A){var C={};
MW.$.each(B,function(E,G){var D=A(G);
var F=(C[D]=C[D]||[]);
F.push(G)
});
return C
}})
}MW.$.fn.serializeObject=function(){var B={};
...
MyWork.Templates.deprecatedTaskHeader = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div class="mw-header aui-toolbar" data-role="header"><h2>', soy.$$escapeHtml("Personal tasks"), '</h2>');
  MyWork.Templates.toggleButtons(null, output);
  output.append('</div>');
...
MyWork.Templates.noNotifications = function(opt_data, opt_sb) {
  var output = opt_sb || new soy.StringBuilder();
  output.append('<div class="mw-no-notifications"><h1>', soy.$$escapeHtml(opt_data.heading), '</h1><div class="subheading"><p>', soy.$$escapeHtml(opt_data.subheading), '</p></div>', soy.$$escapeHtml(opt_data.body), '</div>');
  return opt_sb ? '' : output.toString();
};
...
MW.$.ajax({url:MW.contextPath+"/rest/mywork/latest/task/"+A.get("id")+"/position",type:"PUT",contentType:"application/json",data:JSON.stringify({before:B})})
...
if(this.get("notes")!=A){MW.$.ajax({url:MW.contextPath+"/rest/mywork/latest/task/"+B.get("id")+"/notes",type:"PUT",contentType:"application/json",data:JSON.stringify(A)});
this.set({notes:A})
...
MW.$.ajax({type:"POST",url:MW.contextPath+"/rest/mywork/1/action",data:JSON.stringify(C),dataType:"json",success:B.success,error:B.error,contentType:"application/json"})
...
MW.$.ajax({url:MW.contextPath+"/rest/mywork/latest/notification/"+C+"/status",type:"PUT",contentType:"application/json",data:JSON.stringify(A),success:function(E){D&&D();
...
A=_.map(A,function(D){var C=MW.$.extend(D,{globalEvents:B.globalEvents,parent:B});
...
},i18nHtml:function(A,F){var E=MW.escapeHTML(this.i18nWithMap(A,MW.$.extend({user:"{user}"},F)));
if(!E){return""
}var D=this.getMetadata();
var C=this.config.getUserUrl(this.get("application"),D.username);
var B=D.user||"";
var G=C?MW.$("<a />").attr("target","_blank").attr("href",C).text(B).wrap("<div />").parent().html():MW.escapeHTML(B);
...
},fetchOnce:_.once(function(){var A=MW.$.Deferred();
...
}),fetcher:function(A){this.fetch(this.hasFetched.resolveOnSuccess(_.extend({add:true,data:{type:"inline_task"}},A)))
...
MW.$.ajax({url:MW.contextPath+"/rest/mywork/latest/notification/lastreadid",type:"PUT",contentType:"application/json",data:JSON.stringify(this.currentLastFetchId)})
...
if(B.length>0){MW.$.ajax({url:MW.contextPath+"/rest/mywork/latest/notification/read",type:"PUT",contentType:"application/json",data:JSON.stringify(B)})
...
},render:function(){var A=MW.$(this.el);
...
},isShowingAll:function(){return !MW.$(".mw-show-more").visible()
},render:function(){var A=MW.$(this.el);
this.renderTemplate(A);
...
}var B=MW.$(MyWork.Templates.showMore());
...
A.preventDefault()
},showMoreNotifications:function(D){var C=MW.$(".more-trigger");
var B=MW.$('<li class="mw-loading-more" />');
C.before(B);
...
if(MW.Notifications.isMyWorkAuth(C.get("parent").attributes)){A.find(".mw-drilldown-excerpt").append(MW.$('<button class="authorise">'+"Authorise"+'</button><button class="discard">'+"Discard"+"</button>"))
...
this.$el.find("a").each(function(E,C){var D=MW.$(C).attr("href");
if(!!D&&D.indexOf("/")===0){MW.$(C).attr("href",B+D)
}})
},rerenderFocus:function(){MW.$(this.el).toggleClass("focused",this.model.get("drilldownFocused")||false)
},focusSelected:function(){this.globalEvents.trigger("focusClickedNotification",this.model)
},htmlTitle:function(){var B=this.model.get("parent").entityI18n("this")||"this",A=MW.$.extend({title:B},this.model.get("metadata"));
return this.model.get("parent").i18nHtml("title",A)||MW.escapeHTML(this.model.get("title"))
},ensureNoMissingImage:function(B,A){B.error(function(){var C=MW.$("<span />");
C.addClass(A);
B.after(C);
B.remove()
})
},addActions:function(){var B=MW.$(this.el),F=this.model,E=B.find(".mw-drilldown-actions ul"),A=B.find(".action-form"),C=F.get("actions").reverse();
var D=this;
MW.$.each(C,function(H,I){if(!MW.Configurations.getFromNotification(F.attributes)){return 
...
window.open(A)
},render:function(){var K=MW.$(this.el),F=this,B=this.model,I=this.model.notifications;
...
}this.showCollapsedGroup(MW.$(".collapsed-group"),B);
A.remove()
...
},updateNotes:function(B){var A=this.convertLinks(MW.$("<pre />").text(B).html());
...
this.undelegateEvents();
this.remove()
}},render:function(){var B=MW.$(this.el);
...
},renderTagPlaceholder:function(){if(!!AJS.DarkFeatures&&AJS.DarkFeatures.isEnabled("nat.tags")){MW.$(".mw-taskdetail-container").append(MyWork.Templates.taskDetailTags({task:this.model.attributes}));
this.renderTags()
}},renderTags:function(){if(!!AJS.DarkFeatures&&AJS.DarkFeatures.isEnabled("nat.tags")){var E=this.getTitleText(),C=this.notesTextarea.val(),F=E+" "+C,D=/(^|\s)#(\w+)/g,B=F.match(D),A=MW.$(this.el).find(".mw-taskdetail-taglist");
A.empty();
...
}},addOneTask:function(C,B){MW.$(".mw-no-tasks").hide();
...
if(I.instant){J()
}},addDots:function(){var A=MW.$(this.el).find("a");
var B=A.text();
A.text(B+"...");
return function(){A.text(B)
}
},clickEvent:function(C){var A=this.model.type,B=this;
if(this.disabled){return false
}if(A!=="link"&&A!=="taskLink"){C.preventDefault()
}if(A==="ajax"){this.makeAjaxRequest(null,null,null)
}if(A==="form"){var D=this.options.actionFormElement;
D.find("form").remove();
D.append(this.renderForm());
D.parent().addClass("expanded");
this.showForm(D)
}},toggleDisplayName:function(A,E,D){var C;
if(!!D){C=MW.$(this.el).find("a")
}else{C=MW.$(this.el);
C.unbind()
}var B=(C.text()===A)?E:A;
C.text(B)
...
MW.$(this.el).toggleClass("disabled",A)
...
var C=this,B=this.getFormContainerTemplate(),A=MW.$(B);
...
return A
},render:function(){var E=MW.$(this.el);
E.empty();
...
C=F+MW.formatString(this.model.url,Mw.$.extend({globalId:G.globalId},G.metadata))
}var D=this.actionI18n("displayName")||"Open";
var A=MW.$(MyWork.Templates.openLinkAction({notification:G,url:C,displayName:D}));
A.mouseup(function(){H.get("parent").trigger("open",H)
});
E.append(A)
}else{if(this.model.type==="taskLink"){E.append(MW.$("<a>").attr("href",this.options.url).attr("target","_blank").text(this.actionI18n("displayName")))
}else{E.append(MW.$("<a href='#'>"+this.actionI18n("displayName")+"</a>"))
...
MW.HeaderAbstractView=Backbone.View.extend({events:{"click .mw-task":"selectTask"},initialize:function(A){this.globalEvents=A.globalEvents
},removeBindings:function(){this.$el.unbind();
this.model.off(null,null,this)
},render:function(){var E=MW.$(this.options.el),D=this.options.model,C=this.htmlTitle();
if(!C){return 
...
var A=MW.$.extend({},D.attributes,{title:C,newActionsText:this.newActionsText(),renderTask:B,hasTask:D.hasTask()});
...
},addActions:function(){var G=this,E=this.model,C=this.model.notifications[0],D=MW.$(this.options.el);
var B=MW.$(".drilldown-header-title > .mw-drilldown-actions"),A=D.find(".mw-drilldown-actions > ul");
...
}},_getRelatedTaskButton:function(){return MW.$(this.options.el).find(".drilldown-task-container .mw-task")
...
MW.$(function(){MW.Router=Backbone.Router.extend({initialize:function(A){this.globalEvents=A.globalEvents
...
_.extend(A,{replace:MW.$(".miniview-page").hasClass("iframe")});
return A
...
MW.CardAnimation.showCard(MW.$(this.el),MW.Slide.FROM_LEFT,A,function(){MW.App.scrollToFocusedMain()
})
}});
...
},renderDrilldownTemplate:function(A){MW.$("#mw-container").append(A.render().el);
...
},addTime:function(A,B){A.prepend(MW.$("<li />").text(B))
...
},showCardAnimation:function(A){MW.CardAnimation.showCard(MW.$(this.el),MW.Slide.FROM_RIGHT,A)
},showCardAnimationReverseSlide:function(A){MW.CardAnimation.showCard(MW.$(this.el),MW.Slide.FROM_LEFT,A)
...
MW.makeTextAreaExpandable(A,function(){MW.App.scrollToFocusedElement(".mw-drilldown-container",".mw-notification-drilldown",MW.$(".inline-action-submit"))
});
return A
...
MW.DeprecatedTasksView=Backbone.View.extend({tagName:"div",className:"mw-tasks card",events:{"click .mw-button-notifications":"gotoNotifications","click #mw-deprecated-task-show-me":"closeAndShowFeatureDiscovery","click #mw-deprecated-task-dissmiss":"close"},initialize:function(A){this.globalEvents=A.globalEvents;
this.globalEvents.bind("showTaskCard",this.showTaskCard,this);
...
MW.CardAnimation.showCard(MW.$(this.el),MW.Slide.FROM_LEFT,A)
},renderTemplate:function(A){A.html(MyWork.Templates.deprecatedTasks({userMustMigrate:this.userMustMigrate}));
A.find(".mw-button-tasks").toggleClass("on",true)
},gotoNotifications:function(){this.globalEvents.trigger("showNotificationCard",1)
},closeAndShowFeatureDiscovery:function(A){this.parentCloseAndDiscoverMyTasks();
A.preventDefault()
},close:function(A){this.parentClose();
A.preventDefault()
},migrateIfNeeded:function(){var A=this;
if(A.userMustMigrate){AJS.$.ajax({type:"POST",contentType:"application/json",data:{},url:MW.contextPath+"/rest/mywork/latest/task/migrateTasks"}).done(function(){A.userMustMigrate=false;
A.render()
})
}else{AJS.$.ajax({type:"POST",contentType:"application/json",data:{},url:MW.contextPath+"/rest/mywork/latest/task/hasInteracted"}).done(function(){A.userMustMigrate=false;
A.render()
})
}},parentCloseAndDiscoverMyTasks:function(){window.parent.MW.Tasks.closeAndDiscoverMyTasks()
...
MW.$(function(){var A=_.extend({},Backbone.Events);
MW.globalEvents=A;
...
MW.AppView=Backbone.View.extend({el:MW.$("#mw-container"),count:0,render:function(){},initialize:function(D){var E=this;
this.globalEvents=D.globalEvents;
this.renderNotificationMain();
var C=(MW.$("meta[name='mw-user-has-interacted-with-deprecated-task-mw']").attr("content")==="true");
if(C){MW.$(".mw-toggles.toolbar-group").addClass("hide")
}else{this.renderDeprecatedTaskMain()
...
MW.onJsonMessage(function(G){if(G.unread){F(G.unread)
}else{if(G.markAllRead){E.globalEvents.trigger("markAllRead")
}}});
function F(G){MW.Notifications.fetcher({success:function(){AJS.EventQueue.push({name:"notificationtabopened",properties:{tasks:MW.Tasks.length,unread:G,loadedBy:MW.App.parseView()}});
E.gotoInitial(G)
}});
MW.Tasks.fetchOnce();
E.globalEvents.trigger("reopenMiniview")
}if(MW.parentConfig.preload){MW.$(window.top).focus()
}MW.Configurations.fetchOnce().done(function(){if(MW.parentConfig.preload){return 
}MW.Notifications.fetcher();
MW.Tasks.fetchOnce()
});
MW.Tasks.bind("destroy",function(G){MW.Notifications.setStatusByGlobalId(G.get("globalId"),"")
})
},renderNotificationMain:function(){var C=new MW.NotificationMainView({globalEvents:this.globalEvents});
MW.$("#mw-container").append(C.render().el)
},renderDeprecatedTaskMain:function(){var C=new MW.DeprecatedTasksView({globalEvents:A});
MW.$("#mw-container").append(C.render().el)
},gotoInitial:function(){if(MW.parentConfig.unread=="0"&&MW.Cookie.load()){if(!MW.$(".mw-tasks").is(":visible")){MW.App.gotoTasks(1)
}}else{if(!MW.$(".mw-notifications").is(":visible")){MW.App.gotoMain(1)
...
}}},scrollToFocusedElement:function(D,G,F){var C=MW.$(D),H=MW.$(G||D);
var E=F||MW.$(C).find(".focused");
...
},scrollToDrilldownHeader:function(){this.scrollToFocusedElement(".mw-drilldown-container",".mw-drilldown-container",MW.$(".mw-drilldown-header"))
...
MW.parentConfig={};
if(window.top!=window.self){MW.$(".miniview-page").addClass("iframe");
MW.$("body").bind("click",function(C){C.stopPropagation()
});
...
var A=function(){MW.$(document).bind("keydown",function(F){if(F.which==27){parent.postMessage("escKey","*")
}});
MW.$(document).bind("keypress",function(G){var F=E.keyPressed(G.which);
