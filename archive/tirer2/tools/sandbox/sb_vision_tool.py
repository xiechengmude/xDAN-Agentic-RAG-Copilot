        ...
        try:
            ...
            try:
                if isinstance(self.sandbox, Sandbox):
                    ...
                    if not await self.sandbox.files.exists(full_path, user='root'):
                        return self.fail_response(f"Path '{cleaned_path}' does not exist.")
                    ...
                    if result.exit_code != 0:
                        self.fail_response(f"Path '{cleaned_path}' is a directory, not an image file.")
                    stat = await self.sandbox.commands.run(cmd=f"stat -c%s {full_path}", timeout=0, user='root')
                    file_size = int(stat.stdout.strip())
                    if file_size > MAX_IMAGE_SIZE:
                        size_mb = file_size / (1024 * 1024)
                        max_mb = MAX_IMAGE_SIZE / (1024 * 1024)
                        return self.fail_response(
                            f"Image file '{cleaned_path}' is too large ({size_mb:.2f}MB). "
                            f"Maximum is {max_mb:.2f}MB."
                        )
                else:
            ...
            try:
                if isinstance(self.sandbox, Sandbox):
                    ...
                    image_bytes = await self.sandbox.files.read(full_path, format='bytes')
                else:
