    ...
    async def _execute_browser_action(self, endpoint: str, params: dict = None, method: str = "POST") -> ToolResult:
        ...
        try:
            ...
            if isinstance(self.sandbox, Sandbox):
                response = self.sandbox.process.exec(curl_cmd, timeout=300)
                exec_result = response.result
            elif isinstance(self.sandbox, AsyncSandbox):
                response = await self.sandbox.commands.run(curl_cmd, timeout=0, user='root')
                exec_result = response.stdout
            else:
