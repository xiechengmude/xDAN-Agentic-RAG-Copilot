    ...
    async def expose_port(self, port: int) -> ToolResult:
        try:
            ...
            if isinstance(self.sandbox, Sandbox):
                ...
                url = self.sandbox.get_preview_link(port)
            else:
                raise Exception(f"Invalid sandbox type: {type(self.sandbox)}")

            return self.success_response({
                "url": url,
                "port": port if not isinstance(self.sandbox, AsyncSandbox) else 80,
