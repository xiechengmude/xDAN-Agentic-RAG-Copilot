...
from sandbox.provider.sandbox_provider import SandboxToolsBase
    ...
    async def _file_exists(self, path: str) -> bool:
        ...
        try:
            if isinstance(self.sandbox, Sandbox):
                ...
                return True
            elif isinstance(self.sandbox, AsyncSandbox):
                return await self.sandbox.files.exists(path)
            else:
                raise Exception(f'unknown sandbox type{type(self.sandbox)}')
        except Exception:
            return False
    ...
    async def get_workspace_state(self) -> dict:
        ...
        try:
            ...
            if isinstance(self.sandbox, Sandbox):
                ...
                files = await self.sandbox.files.list(self.workspace_path)
            else:
                raise Exception(f"Unsupported sandbox Type: {type(self.sandbox)}")
            ...
            for file_info in files:
                ...
                try:
                    ...
                    if isinstance(self.sandbox, Sandbox):
                        ...
                        content = await self.sandbox.files.read(full_path)
                    else:
                        raise Exception(f"Unsupported sandbox type: {type(self.sandbox)}")
        ...
        try:
            ...
            if parent_dir:
                if isinstance(self.sandbox, Sandbox):
                    ...
                    await self.sandbox.files.make_dir(parent_dir, user='root')
            ...
            if isinstance(self.sandbox, Sandbox):
                ...
                await self.sandbox.files.write(full_path, file_contents.encode())
            
            message = f"文件'{file_path}'创建成功。"
            
            # 上传到S3并生成公共URL
            try:
                from utils.s3_upload_utils import upload_base64_file
                
                # 获取文件名
                filename = os.path.basename(file_path)
                
                # 上传文件并获取公共URL
                public_url = await upload_base64_file(
                    base64_data=file_contents,
                    filename=f"{self.sandbox_id}/{filename}",
                )
                
                # 添加URL信息到响应消息
                # message += f"\n文件可通过以下URL访问: {public_url}"
                logger.info(f"文件可通过以下URL访问: {public_url}")
                
            except Exception as s3_err:
                logger.warning(f"上传到S3失败: {str(s3_err)}")
                message += f"\n警告: 无法上传到外部存储: {str(s3_err)}"
    ...
    async def str_replace(self, file_path: str, old_str: str, new_str: str) -> ToolResult:
        try:
            ...
            if isinstance(self.sandbox, Sandbox):
                ...
                await self.sandbox.files.write(full_path, new_content.encode())
            else:
                raise Exception(f"Unsupported sandbox type: {type(self.sandbox)}")
