...
class SandboxShellTool(SandboxToolsBase):
    ...
    async def _execute_raw_command(self, command: str) -> Dict[str, Any]:
        ...
        if isinstance(self.sandbox, Sandbox):
            from sandbox.provider.daytona_sandbox import SessionExecuteRequest
                # Ensure session exists
            session_id = await self._ensure_session("raw_commands")
                # Execute command in session
            req = SessionExecuteRequest(
                command=command,
                runAsync=False,  # This makes the command blocking by default
                cwd=self.workspace_path  # Still set the working directory for reference
            )
            ...
            response = await self.sandbox.commands.run(cmd=command, user='root', timeout=0)
            logs = response.stdout or response.stderr
        else:
