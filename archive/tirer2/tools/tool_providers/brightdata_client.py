    ...
    async def search(self, query: str, **kwargs) -> Dict[str, any]:
        ...
        async with self.semaphore:
            ...
            try:
                ...
                logger.debug(f"BrightData搜索输入: query={query}, url={search_url}")
                ...
                async with self.session.post(
                    ...
                    logger.debug(f"BrightData搜索成功: query={query}, 结果数={len(results) if isinstance(results, list) else 1}, 耗时={elapsed:.2f}s")
                ...
                logger.error(f"BrightData搜索失败: {e}, query={query}")
                return {
                    'success': False,
                    'query': query,
                    'error': str(e),
