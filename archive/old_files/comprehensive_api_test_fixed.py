...
class ComprehensiveAPITester:
    ...
    async def test_upload_document(self, dataset_id: str):
        ...
        try:
            ...
            # 上传文件时不要包含Content-Type头，让aiohttp自动设置
    ...
    async def test_delete_dataset(self, dataset_id: str):
        ...
        try:
            async with self.session.delete(f"{self.base_url}/api/v1/datasets/{dataset_id}", headers=self.headers) as response:
                data = await response.json()
                await self.log_test_result(
                    f"删除数据集({dataset_id})", 
                    response.status == 200,
    ...
    async def generate_test_report(self, total_tests: int, passed_tests: int):
        ...
        with open('api_test_report_final.json', 'w', encoding='utf-8') as f:
            json.dump(report_data, f, ensure_ascii=False, indent=2)
            
        logger.info("\n详细测试报告已保存到: api_test_report_final.json")
...
async def main():
    """主函数"""
    logger.info("开始执行完整的API接口测试 - 修复版本")
