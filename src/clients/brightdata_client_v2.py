...
class BrightDataAsyncClient:
    """高性能异步BrightData客户端 - 优化版本支持SERP API"""
        ...
        async with self.semaphore:
            ...
            try:
                ...
                async with self.session.post(
                    ...
                    if response_id and kwargs.get("async_mode", False):
                        # 异步模式：轮询结果
                        results = await self._poll_async_result(response_id)
                    else:
                        # 同步模式：直接获取结果
                        results = await response.json()
                    
                    # 处理结果
                    processed_results = self._process_serp_results(results)
    ...
    async def multi_search(self, queries: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        使用BrightData multi参数进行并行搜索
        
        Args:
            queries: 查询列表，每个包含query和options
                     例：[{"query": "AI research", "options": {"num_results": 10}}]
        ...
        async with self.semaphore:
            ...
            try:
                ...
                for q_data in queries:
                    ...
                    params = self._build_serp_params(query, options)
                    multi_queries.append({"query": params})
                
                data = {
                    "country": queries[0].get("options", {}).get("country", "us").lower(),
                    "multi": multi_queries
                }
                ...
                async with self.session.post(
                    ...
                    if isinstance(results, list):
                        for i, result in enumerate(results):
                            processed_result = self._process_serp_results(result)
                            processed_results.append({
                    ...
                    elapsed = time.time() - start_time
                    logger.info(f"Multi搜索完成: {len(queries)}个查询, 耗时: {elapsed:.2f}s")
                ...
                logger.error(f"Multi搜索错误: {str(e)}")
                return {
                    'success': False,
                    'queries': [q["query"] for q in queries],
                    'error': str(e),
    ...
    def _process_serp_results(self, results: Dict) -> List[Dict]:
        ...
        if 'news_results' in results:
            for i, news in enumerate(results['news_results']):
                processed_results.append({
...
async def example_usage():
    ...
    async with BrightDataAsyncClient(
        ...
        if result['success']:
            print(f"SERP搜索找到 {len(result['results'])} 个结果")
            for item in result['results'][:3]:
                print(f"- {item['title']}")
                print(f"  {item['url']}")
        
        # 2. 分页搜索
        page2_result = await client.search(
            "AI research trends",
            page=2,  # 第2页
            num_results=10
        )
        print(f"\n第2页结果: {len(page2_result['results']) if page2_result['success'] else 0}")
        
        # 3. 并行multi搜索（性能提升3-5倍）
        queries = [
            {"query": "S3 framework search", "options": {"num_results": 10}},
            {"query": "LLM routing strategies", "options": {"search_type": "news"}},
            {"query": "RAG optimization", "options": {"date_range": "m"}}
        ]
        multi_result = await client.multi_search(queries)
        if multi_result['success']:
            print(f"\nMulti搜索完成: {multi_result['count']}个查询并行执行")
            for result in multi_result['results']:
                print(f"  {result['query']}: {len(result['results'])}个结果")
        
        # 4. 地理位置搜索
        local_result = await client.search(
            "best restaurants",
            location="New York,New York,United States",
            num_results=15
        )
        print(f"\n本地搜索: {'成功' if local_result['success'] else '失败'}")
        
        # 5. 移动设备搜索
        mobile_result = await client.search(
            "mobile shopping apps",
            device="mobile",
            search_type="shopping"
        )
        print(f"\n移动搜索: {'成功' if mobile_result['success'] else '失败'}")
